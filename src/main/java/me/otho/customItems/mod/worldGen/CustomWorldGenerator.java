...
public class CustomWorldGenerator implements IWorldGenerator {
        ...
        if (oresToSpawn != null) {
            for (i = 0; i < oresToSpawn.length; i++) {
                ...
                if (oreInfo.dimensionId == world.provider.dimensionId) {
                    ...
                    if (oreInfo.biomeId == null || Arrays.asList(oreInfo.biomeId)
                        .contains(biome.biomeID)) {
                        this.addOreSpawn(
                            oreBlock,
                            toSpawnMetadata,
                            toReplace,
    ...
    public void addOreSpawn(Block block, int metadata, Block toReplace, World world, Random random, int blockXPos,
